...
fn create_client_confirm_active(
    ...
    if config.rail.is_some() {
        ...
        // These used to be inlined as raw bytes here because IronRDP modelled the bodies as
        // opaque; upstream now provides typed capability structs, so use those instead. Same
        // wire bytes, but the field meanings are checked by the compiler.
        server_capability_sets.push(CapabilitySet::Rail(Rail {
