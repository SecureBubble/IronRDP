...
fn create_gcc_blocks<'a>(
    ...
    Ok(ClientGccBlocks {
        ...
        cluster: None,
        // Upstream's gating is correct and replaces our parallel `config.monitors` API:
        // GCC Client Monitor Data is only legal when Extended Client Data is supported.
...
fn create_client_info_pdu(
    ...
    // RAIL (Remote Programs) mode: the server runs published applications rather than a desktop
    // shell, and drives the `rail` static channel. Upstream signals this with the
    // `remote_application_mode` flag; our fork signals it by supplying a `rail` launch config for
    // the `ironrdp-rdperp` crate. Honour BOTH so either consumer works.
    if config.remote_application_mode || config.rail.is_some() {
