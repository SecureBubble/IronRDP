...
fn decode_vbar<'a>(src: &mut ReadCursor<'a>, band_height: u16) -> DecodeResult<VBar<'a>> {
    ...
    // Per MS-RDPEGFX 2.2.4.1.1.3.1.2.2 (SHORT_VBAR_CACHE_MISS), matching the FreeRDP
    // reference decoder (libfreerdp/codec/clear.c: `vBarYOn = header & 0xFF`,
    // `vBarYOff = (header >> 8) & 0x3F`):
    //   bits 7:0  = shortVBarYOn  (8 bits): first pixel row with color data
    //   bits 13:8 = shortVBarYOff (6 bits): row where color data ends
...
mod tests {
    ...
    fn decode_vbar_short_cache_miss() {
        ...
        // Wire layout: yOn in bits 7:0, yOff in bits 13:8.
        let y_on: u16 = 2;
        let y_off: u16 = 5;
